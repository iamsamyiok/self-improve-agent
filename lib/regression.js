...
function runFullSmoke(dir) {
  // 测试隔离（短板中9）：剔除 PORT，避免外层 smoke 的 e2e 服务器端口残留进子进程；
  // 子 smoke 的端口区间错开（DUAL_AGENT_SMOKE_PORT 传独立随机值），防两代 smoke 撞端口
  const env = { ...process.env };
  delete env.PORT;
  env.DUAL_AGENT_SMOKE_PORT = String(4100 + Math.floor(Math.random() * 200));
