...
const README_MAX = 8000;     // README 截断（省 token，机制信息集中在头部）
...
function savePendingGene(repo, mech, adapt) {
  ...
  const gene = { id: nowId('sg'), text, source: `github:${repo}`, mechanism: String(mech.name || '').slice(0, 80), status: 'pending', createdAt: new Date().toISOString() };
  db.genes.push(gene);
...
module.exports = { runScout, scoutStatus, scoutDue, discovery, fetchReadme, extractMechanisms, adaptMechanism, validateSkill, pendingGenesPromptSection, markGeneValidated, DAILY_GOAL };
