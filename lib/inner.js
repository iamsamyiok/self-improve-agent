...
async function chatInnerReal(cfg, messages, tools, callPlugin, onEvent, opts = {}) {
  ...
  while (round < totalRounds) {
    ...
    // 网络抖动按 3s→9s→27s→81s 指数退避（±50% 抖动）自动重试；重试时重置本轮累积（assistant
    // 消息尚未入 messages，text 事件为快照式，重复安全）；429 带 Retry-After 时优先服务端指示
    const { content, calls, usage } = await withRetry(async () => {
      ...
      if (!resp.ok || !resp.body) {
        ...
        if (isRetryableStatus(resp.status) || isRateLimitText(txt)) {
          const err = new RetryableError(`API ${resp.status}：${txt.slice(0, 160) || '无响应体'}`);
          // 服务端 Retry-After 指示（秒或 HTTP 日期）：优先于客户端指数序列
...
module.exports = { chatInner, chatInnerReal, MAX_ROUNDS, READONLY_PLUGINS, sanitizeToolArguments, parseToolArgs, reassembleCalls, shouldStall, recordFail, STALL_LIMIT, budgetMessages, estimateChars, estimateTokens, estimateTokensV2, estimateMessagesTokens, usageNoteMsg, isMultiStepTask, isLongFormTask, isRefusalNudge, pairSafeTail, parseHermesToolCalls, parseKwargsArgs };
