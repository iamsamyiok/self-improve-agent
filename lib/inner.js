...
function estimateMessagesTokens(messages) {
  // memo：budgetMessages 内部已算过一次的结果直接复用（非枚举属性，不污染落盘/序列化）
  if (messages && messages.__tokens) return messages.__tokens;
  let total = 0;
  for (const m of messages) {
    if (m.role === 'system') {
...
function budgetMessages(messages) {
  ...
  if (tokens <= budget) return withTokensMemo(messages, tokens);
  ...
  return withTokensMemo(out, estimateMessagesTokens(out));
}

// 把估算结果挂到数组上（非枚举属性）：chatInner 每轮 budgetMessages 已全量扫一遍，
// 紧接着的 estimateMessagesTokens(budgeted) 直接复用，省一次全量逐字符扫描
function withTokensMemo(arr, tokens) {
  try { Object.defineProperty(arr, '__tokens', { value: tokens, enumerable: false, configurable: true }); } catch { /* 冻结数组等场景忽略 */ }
  return arr;
}
...
async function chatInnerReal(cfg, messages, tools, callPlugin, onEvent, opts = {}) {
  ...
  while (round < totalRounds) {
    ...
    if (!roundUsage) {
      est = true;
      roundUsage = {
        prompt_tokens: estimateMessagesTokens(sendMsgs) + (tools.length ? estimateTokensV2(JSON.stringify(tools)) : 0),
