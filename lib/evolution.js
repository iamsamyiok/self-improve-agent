...
function buildMutationPrompt(cases) {
  ...
  return [
    ...
    '3. skill：修改或新建 skills 下的技能文件，target=skills/<目录>/<文件>.md（新建目录也可），change=该文件完整新内容。技能是显式知识，语义清晰、可直接审查——这是信息量最大的 mutation 类型。',
...
async function runCase(exp, c, candidateDir, opts) {
  ...
  return {benchmark:c.id,baseline:b,candidate:cnd};
}
...
async function runEvolution(opts={}) {
  ...
  for (const c of selected) {
    ...
    row.evaluation=await evaluateCase(c,row.baseline,row.candidate,cfg);
  ...
  const decision={experiment:id,mutation,summary,holdout,earlyStop,prefilterReject,createdAt:new Date().toISOString()};
...
async function growPool(opts={}) {
  ...
  let taskList;
  if (process.env.DUAL_AGENT_MOCK === '1') {
    taskList = [{ task: `创建 curriculum-mock.txt，写入 3 行自动课程生成内容（第 1 行固定为 curriculum-ok）`, acceptance: ['文件存在', '第 1 行为 curriculum-ok'] }];
  } else {
  ...
  for (const t of taskList.slice(0, n)) {
    ...
    const rec = recordBenchmark({ task, intent, finalText: r.finalText, artifacts: r.artifacts || [], ws: 'auto-curriculum' });
...
module.exports={recordBenchmark,artifactManifest,runEvolution,status,history,liveStatus,currentSignature,shouldAutoEvolve,shouldAutoPromote,EV_ROOT,listGaps,rankHardFirst,recordGap,checkEarlyStop,recordLessons,listRelevantLessons,lessonsPromptSection,growPool,splitPool};
