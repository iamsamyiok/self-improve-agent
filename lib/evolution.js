...
function buildMutationPrompt(cases, emphasizeFailureTargeting) {
...
async function proposeMutation(cases, opts={}) {
  ...
  let mutation = await gen(buildMutationPrompt(cases));
  if (failureModes.length && mutation.type && mutation.type !== 'none' && !targetsFailureModes(mutation, failureModes)) {
    mutation = await gen(buildMutationPrompt(cases, true));
  }
  return mutation;
}
