...
async function evaluateCase(caseRec, baseRun, candRun, cfg, paths) {
  ...
  const msgs = [
    ...
    {role:'user',content:[
      ...
      `Baseline 最终回答：${String(baseRun.finalText||'').slice(0,3000)}`,
      `Candidate 最终回答：${String(candRun.finalText||'').slice(0,3000)}`,
      `Baseline 工具指标：${JSON.stringify(slimMetrics(baseRun.metrics))}`,
      `Candidate 工具指标：${JSON.stringify(slimMetrics(candRun.metrics))}`,
...
async function runEvolution(opts={}) {
  ...
  try {
    const usedNow = {};
    const now = Date.now();
    for (const c of [...selected, ...holdoutCases]) usedNow[c.id] = now;
    writeJson(STATE, { ...stateForUsage, caseUsage: { ...(stateForUsage.caseUsage || {}), ...usedNow } });
  } catch { /* 轮换记录失败不影响实验 */ }
  ...
  const PAR = Math.max(1, Math.min(4, Number(process.env.DUAL_AGENT_EVOLUTION_PARALLELISM) || 2));
  let prefilterChecked = false;
  for (let wi = 0; wi < selected.length; wi += PAR) {
    if (earlyStop || prefilterReject) break;
    const wave = selected.slice(wi, wi + PAR);
    const rows = await Promise.all(wave.map(async c => {
  ...
  const decision={experiment:id,mutation,targetedPatterns:analyzeFailureModes(3),summary,holdout,earlyStop,prefilterReject,semanticRecall:SEMANTIC_RECALL_COUNT,createdAt:new Date().toISOString()};
