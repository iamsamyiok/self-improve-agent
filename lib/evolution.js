...
function recordBenchmark(input) {
  ...
  if (rec.hard) recordGap({ ts: rec.createdAt, task: rec.task.slice(0, 2000), acceptance: rec.acceptance, gaps: lastGaps, repairs });
...
async function runEvolution(opts={}) {
  ...
  const selected = ranked.slice(0, Math.min(ranked.length, Number(opts.cases)||12));
  ...
  for (const c of selected) fs.mkdirSync(path.join(exp,'cases',c.id),{recursive:true});
  ...
  const summary=aggregate(results); const decision={experiment:id,mutation,summary,earlyStop,createdAt:new Date().toISOString()};
  ...
  return {ok:true,stage:summary.pass?(opts.promote?'promoted':'ready'):'rejected',experiment:id,mutation,summary,earlyStop,results:results.map(r=>({benchmark:r.benchmark,evaluation:r.evaluation}))};
}
...
module.exports={recordBenchmark,artifactManifest,runEvolution,status,history,liveStatus,currentSignature,shouldAutoEvolve,shouldAutoPromote,EV_ROOT,listGaps,rankHardFirst,recordGap,checkEarlyStop};
