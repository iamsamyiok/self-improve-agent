...
function recordTaskOutcome(rec) {
  try {
    ...
    const row = { ts: Date.now(),
      ...
      // CoE 组合反馈（arXiv 2608.18027）：模型自评完成度与背离标记，供健康分析分维度观察
      selfReview: rec.selfReview && typeof rec.selfReview.completeness === 'number' ? { c: rec.selfReview.completeness, d: !!rec.selfReview.divergent } : null,
      task: String(rec.task || '').slice(0, 200) };
    fs.appendFileSync(EVAL_EVENTS, JSON.stringify(row) + '\n');
    // 容量截断挪到 cleanupStale（每 24h）：此前每次任务边界全量读+重写整个 jsonl（O(文件)，
    // 2000 条 ≈ 200KB），abort/undo 时同步阻塞请求路径；parseEvalEvents 读取侧本就自截断
    // 强化处理挂钩：失败/困难任务进补练队列（运行时懒加载避免循环依赖；容错不抛）
    try { require('./reinforce').onTaskOutcome(row); } catch { /* 强化入队失败不影响 */ }
  } catch { /* 评估采集失败不影响任务交付 */ }
}
...
function listAssets(limit) {  const n = Number(limit) || 8;
...
// 进化中心明细（统计格点击展开）：样本 / 实验 / 晋级 三类
// 过期档案：完结超清理阈值（30 天）cases/ 已被 cleanupStale 清除的实验——只保留基本行，结论内容不再展示
function listEvoDetail(type, limit) {
  const n = Math.min(50, Number(limit) || 20);
  if (type === 'benchmarks') {
    return listBenchmarks().slice(0, n).map(b => ({
      id: b.id,
      task: String(b.task || '').slice(0, 70),
      hard: !!b.hard,
      divergent: !!(b.selfReview && b.selfReview.divergent),
      repairs: b.repairs || 0,
      createdAt: b.createdAt || ''
    }));
  }
  if (type === 'experiments') {
    let dirs = []; try { dirs = fs.readdirSync(EXPS).filter(d => d.startsWith('exp-')).sort(); } catch { return []; }
    return dirs.slice(-n).reverse().map(name => {
      const dir = path.join(EXPS, name);
      const d = readJson(path.join(dir, 'decision.json'), null);
      const p = readJson(path.join(dir, 'proposal.json'), null);
      const expired = !!d && !fs.existsSync(path.join(dir, 'cases')); // 完结且执行痕迹已清 → 过期档案
      return {
        id: name.slice(4),
        date: (d && d.createdAt) || (p && p.createdAt) || '',
        type: expired ? '' : ((p && p.type) || (d && d.mutation && d.mutation.type) || ''),
        pass: expired ? false : !!(d && d.summary && d.summary.pass),
        delta: expired ? null : (d && d.summary ? d.summary.meanDelta : null),
        promoted: !expired && !!(d && d.promotion && d.promotion.ok),
        expired
      };
    });
  }
  if (type === 'promotions') {
    return history(100)
      .filter(x => x.promotion && x.promotion.ok)
      .map(x => ({
        id: x.experiment,
        version: x.promotion.version,
        date: x.createdAt || '',
        delta: x.summary ? x.summary.meanDelta : null,
        what: String((x.explanation && x.explanation.whatChanged) || '').slice(0, 120)
      }));
  }
  return [];
}
...
function cleanupStale(days) {
  ...
  const removed = [];
  // eval-events 滑动窗口截断（recordTaskOutcome 已改纯 append，容量治理集中在此低频路径）
  try {
    const lines = fs.readFileSync(EVAL_EVENTS, 'utf8').split('\n').filter(Boolean);
    if (lines.length > EVAL_EVENTS_MAX) {
      fs.writeFileSync(EVAL_EVENTS, lines.slice(-EVAL_EVENTS_MAX).join('\n') + '\n');
      removed.push(`eval-events 截断 ${lines.length - EVAL_EVENTS_MAX} 条`);
    }
  } catch { /* 无事件文件 */ }
  let dirs = []; try { dirs = fs.readdirSync(EXPS).filter(d => d.startsWith('exp-') || d.startsWith('pool-')); } catch { return removed; }
  // baseline 缓存：按版本指纹组织，版本更替自然失效；7 天 TTL 兜底清理（与实验清理阈值解耦）
  try {
    const cacheRoot = path.join(EV_ROOT, 'baseline-cache');
    const cacheAge = 7 * 24 * 60 * 60 * 1000;
    for (const fp of fs.readdirSync(cacheRoot)) {
      const p = path.join(cacheRoot, fp);
      try { if (now - fs.statSync(p).mtimeMs > cacheAge) { fs.rmSync(p, { recursive: true, force: true }); removed.push('baseline-cache/' + fp); } } catch { /* 单指纹失败不影响 */ }
    }
  } catch { /* 无缓存目录 */ }
...
function rankHardFirst(cases, usage) {
  ...
  // divergent 最优先（CoE 组合反馈，arXiv 2608.18027）：自评与实测背离的样本
  // 暴露的是「两个反馈通道感知不一致」的盲区，区分度高于普通 hard。
  const now = Date.now();
  const isFresh = c => { const t = usage && usage[c.id]; return !(t && now - Number(t) < 24 * 3600 * 1000); };
  const divergent = [], hardFresh = [], hardUsed = [], rest = [];
  for (const c of cases) {
    const h = (c.repairs > 0 || c.hard) ? 1 : 0;
    if (c.selfReview && c.selfReview.divergent && !c.duplicateOf) { divergent.push(c); continue; }
  ...
  return [...byTime(divergent), ...byTime(hardFresh), ...byTime(hardUsed), ...byTime(rest)];
}
...
function recordBenchmark(input) {
  ...
  // CoE 组合反馈（arXiv 2608.18027）：自评与实测背离的样本（自评高但核验未过 / 自评低但零返修 PASS）
  // 是「环境核验与模型自评感知不同失败维度」的直接证据，rankHardFirst 选样时最优先重放
  if (input.selfReview && typeof input.selfReview.completeness === 'number') {
    rec.selfReview = { completeness: input.selfReview.completeness, divergent: !!input.selfReview.divergent };
  }
...
function buildHypothesisContext(cases) {
  ...
  // 拼装上限对齐上下文预算（lib/limits.js 窗口 80% ≈ 58 万字符）：48K 字符仅占 8%，
  // 多带任务池证据提升 mutation 靶向质量；仍远小于预算，防超限只是兜底
  return JSON.stringify({ recent, gapTasks, bottlenecks, audit, recentExperiments }, null, 2).slice(0, 48000);
}
...
// 外部学习候选基因注入：scout 从 GitHub 热门项目迁移来的机制，供 Meta-Agent 优先作为 gene mutation 素材
function pendingGenesSection() {
  try { return require('./scout').pendingGenesPromptSection(); } catch { return ''; }
}

// 强化处理聚类提案注入：补练反复失败的同类任务根因，提示 Meta-Agent 做结构化 mutation
function reinforceSection() {
  try { return require('./reinforce').clustersPromptSection(); } catch { return ''; }
}
...
async function evaluateCase(caseRec, baseRun, candRun, cfg, paths) {
  ...
  const msgs = [
    ...
    {role:'user',content:[
      ...
      `A 最终回答：${String(runA.finalText||'').slice(0,6000)}`,
      `B 最终回答：${String(runB.finalText||'').slice(0,6000)}`,
...
async function runCase(exp, c, candidateDir, opts) {
  ...
  const [b, cnd] = await Promise.all([
    runBaseline(c, baseWs, baseData, common, Array.isArray(baseGenes)?baseGenes:[]),
...
// ===== baseline 缓存（v3.4.3 实验提速）=====
// baseline = 当前版本对任务的真实执行，版本不变时结果可直接复用：相邻实验反复重跑同一批
// 任务的 baseline，白烧近一半 worker 调用。缓存 key = 版本指纹（启用基因+策略+插件集）
// + caseId；命中时恢复工作区产物与结果（标记 cached:true）跳过 baseline worker——
// 单 case 耗时近乎减半。固定对照组同时降低 A/B 方差（配对比较）。
// candidate 永不缓存（每次测的都是新变更）；失败/限流结果不缓存。
function versionFingerprint() {
  const parts = ['fp-v1'];
  try { parts.push(JSON.stringify(enabledGenes().map(g => g.text))); } catch { parts.push('nogenes'); }
  try { const sp = path.join(DATA_DIR, 'strategy.json'); parts.push(fs.existsSync(sp) ? fs.readFileSync(sp, 'utf8') : '{}'); } catch { parts.push('nostrategy'); }
  try { const pd = process.env.DUAL_AGENT_PLUGINS_DIR || path.join(ROOT, 'plugins'); parts.push(fs.readdirSync(pd).sort().map(f => { const st = fs.statSync(path.join(pd, f)); return f + ':' + st.size + ':' + Math.floor(st.mtimeMs / 1000); }).join(',')); } catch { parts.push('noplugins'); }
  return sha(parts.join('|')).slice(0, 12);
}

async function runBaseline(c, baseWs, baseData, common, baseGenes) {
  const cacheDir = path.join(EV_ROOT, 'baseline-cache', versionFingerprint(), c.id);
  const cacheWs = path.join(cacheDir, 'workspace');
  if (fs.existsSync(path.join(cacheDir, 'result.json'))) {
    try {
      const cached = JSON.parse(fs.readFileSync(path.join(cacheDir, 'result.json'), 'utf8'));
      fs.rmSync(baseWs, { recursive: true, force: true });
      fs.cpSync(cacheWs, baseWs, { recursive: true }); // 恢复当时产物，供 objective 核验
      return Object.assign(cached, { cached: true });
    } catch { /* 缓存损坏则重跑 */ }
  }
  const b = await runWorker({...common,workspace:baseWs,dataDir:baseData,pluginsDir:process.env.DUAL_AGENT_PLUGINS_DIR||path.join(ROOT,'plugins'),skillsDir:process.env.DUAL_AGENT_SKILLS_SHARED||path.join(ROOT,'skills'),genes:baseGenes,label:'baseline'});
  if (b && !b.error && b.finalText) {
    try {
      fs.mkdirSync(cacheWs, { recursive: true });
      fs.cpSync(baseWs, cacheWs, { recursive: true });
      fs.writeFileSync(path.join(cacheDir, 'result.json'), JSON.stringify(b), 'utf8');
    } catch { /* 缓存写入失败不影响实验 */ }
  }
  return b;
}
...
async function runEvolution(opts={}) {
  ...
  // "每批"（最多多跑一批），换取实验时长近并发度倍缩短。v3.4.3 默认 4、上限 8（baseline
  // 缓存命中后单 case 只跑 candidate，API 压力减半，并行空间翻倍）；限流自适应降档兜底。
  // case 间无共享状态（各自独立 worker 子进程 + 独立 case 工作区），
  // A/B 公平性已由 worker 路径温度 0 保证；结果按 case 提交顺序回填，aggregate 可复现。
  const PAR_MAX = Math.max(1, Math.min(8, Number(process.env.DUAL_AGENT_EVOLUTION_PARALLELISM) || 4));
...
// 状态聚合（前端 8s 轮询 + 多标签页共享）：单次 status 要全量扫描 benchmarks/eval-events/experiments，
// 实测 10-50ms 同步 IO——加 3s TTL 缓存消除空闲期放大（展示场景 3s 延迟无感）
let _statusCache = { at: 0, data: null };
function status() {
  const now = Date.now();
  if (_statusCache.data && now - _statusCache.at < 3000) return _statusCache.data;
  const state=readJson(STATE,{});
  ...
  const result = {
    ...
    history:hist,
    latest:hist[0] || null  };
  _statusCache = { at: now, data: result };
  return result;
}
...
function liveStatus() {
  ...
  // 区分必跑（训练池）与复验（holdout：仅 A/B 通过后才会执行）——进度只按必跑任务计算，
  // 避免把大概率不会执行的 holdout 计入总数造成"实验很慢"的误导
  const state=readJson(path.join(exp,'state.json'),null);
  const holdoutIds=new Set(state&&Array.isArray(state.holdout)?state.holdout:[]);
  const cases=caseIds.map(id=>({ id, holdout:holdoutIds.has(id), done:fs.existsSync(path.join(casesDir,id,'result.json')) }));
  const trainCases=cases.filter(c=>!c.holdout);
  const doneCount=trainCases.filter(c=>c.done).length;
  ...
  return {
    ...
    cases, total:trainCases.length, done:doneCount,
    holdoutTotal:cases.length-trainCases.length
  };
}
