...
if(Array.isArray(input.genes)) process.env.DUAL_AGENT_EVOLUTION_GENES=JSON.stringify(input.genes);
...
(async()=>{
  try{
    ...
    const toolTrace=rows.slice(-40).map(x=>({plugin:x.plugin,ok:!!x.ok}));
    process.stdout.write(JSON.stringify({ok:!!r.ok,finalText:r.finalText||'',aborted:!!r.aborted,artifacts,metrics,toolTrace})+'\n');
