...
// 2026-09-04 加固：①AbortController 超时（挂起流到时中断转可重试，防任务卡到外层超时）
// ②status 错误统一 makeHttpError（500/502/504 等瞬态错误此前不可重试）③usage 帧捕获经 onUsage
// 上报（进化自身 token 开销可审计；老网关不识别 stream_options 时自动降级）
async function callLLMText(cfg, msgs, opts = {}) {
  const url = String(cfg.base_url || '').replace(/\/+$/, '') + '/chat/completions';
  return await withRetry(async () => {
    const to = new LlmTimeout(opts.label || '意图');
    try {
      const payload = { model: cfg.model, messages: msgs, stream: true, ...(opts.maxTokens ? { max_tokens: opts.maxTokens } : {}) };
      if (!callLLMText._noUsageOpt) payload.stream_options = { include_usage: true };
      ...
      if (!resp.ok || !resp.body) {
        ...
        if (resp.status === 400 && !callLLMText._noUsageOpt && /stream_options/i.test(txt)) {
          callLLMText._noUsageOpt = true;
          throw new RetryableError('网关不识别 stream_options，去除该参数重试');
        }
        throw makeHttpError(resp.status, txt, opts.label || '意图', resp.headers);
      }
      ...
      while (true) {
        ...
        while ((i = buf.indexOf('\n')) >= 0) {
          ...
          if (ev.usage && typeof ev.usage === 'object') usage = ev.usage; // usage 末帧 choices 为空，先于 delta 判定
          const delta = ev.choices && ev.choices[0] && ev.choices[0].delta;
          if (delta && typeof delta.content === 'string') out += delta.content;
        }
      }
      if (usage && typeof opts.onUsage === 'function') { try { opts.onUsage(usage); } catch { /* 计量失败不影响主流程 */ } }
      to.clear();
      return out;
    } catch (e) {
      throw to.settle(e);
